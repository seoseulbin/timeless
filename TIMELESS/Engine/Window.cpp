...
void Window::Init(const char* windowtitle, int width, int height, const SDL_Event& e)
{
    ...
    width = std::max(640, std::min(16384, width));
    height = std::max(480, std::min(16384, height));
    Window_ptr = SDL_CreateWindow(windowtitle, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
